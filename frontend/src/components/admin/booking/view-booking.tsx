...
import type { BookingData } from "./booking-management";
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                ...
                >
                  {booking.paymentMethod === "vnpay" ? "VNPAY" : "MoMo"}
                </span>
              </div>
            </div>
          </div>
                  ...
                  <div>
                    <div className="space-y-2">
                      ...
                      {(() => {
                        ...
                        // Try to get original seat price from API
                        let seatTotal = 0;
                        if (booking.seatPrice && booking.seatPrice > 0) {
                          // Use seat price from API if available
                          seatTotal = booking.seatPrice * booking.seats.length;
                        } else if (booking.originalAmount && booking.originalAmount > 0) {
                          // Use original amount minus concessions if available
                          seatTotal = booking.originalAmount - concessionsTotal;
                        } else {
                          // Fallback: calculate backwards from total amount
                          seatTotal = booking.totalAmount - concessionsTotal + promotionDiscount;
                        }
                        ...
                        return (
                          ...
                          {(() => {
                            const discount = Number(booking.promotionDiscount || 0);
                            
                            if (discount > 0 || booking.promotion) {
                              return (
                                <div className="flex justify-between items-center py-1 border-b border-gray-100">
