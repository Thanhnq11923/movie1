...
const vnpayService = {
    /**
     * Create a booking with VNPay payment method
     * @param bookingData Booking information
     * @returns Response with booking data and payment URL
     */
    createVNPayBooking: async (bookingData: BookingData): Promise<BookingResponse> => {
        // Set payment method to VNPay
        const bookingWithVNPay = {
            ...bookingData,
            paymentMethod: 'vnpay'
        };
        ...
        const res = await fetch('/api/bookings', {
            ...
            body: JSON.stringify(bookingWithVNPay)
        });
    ...
    /**
     * Process the payment by redirecting to VNPay
     * @param paymentUrl The URL returned from the backend to redirect to VNPay
     */
    ...
    /**
     * Update payment status after VNPay payment completion
     * @param bookingId The ID of the booking to update
     * @param status The new payment status ('completed' | 'failed')
     * @param transactionInfo Optional transaction information from VNPay
