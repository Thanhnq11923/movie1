...
// Add a type definition for user data
...
export const userService = {
  ...
  addUser: async (data: UserData) => {
    try {
      ...
      // Đảm bảo memberId luôn được truyền lên
      if (!data.memberId || !data.memberId.trim()) {
        throw new Error("Staff ID is required and cannot be empty");
      }

      // Đảm bảo member object cũng có memberId
      const userData = {
        ...data,
        memberId: data.memberId.trim(), // Đảm bảo không có khoảng trắng
        member: {
          memberId: data.memberId.trim(),  // Đảm bảo nhất quán trong member object
          score: 0  // Giá trị mặc định
        }
      };

      console.log("🚀 Sending payload to /users:", userData); // Debug payload
      const response = await userApi.post("/users", userData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log("✅ Response from /users:", response.data); // Debug response
      return response.data;
    } catch (error: unknown) {
  ...
  /**
   * Get users by roleId (e.g. staff)
   * If roleId is empty, returns all users
   */
  getUsersByRoleId: async (roleId: string) => {
    const token = localStorage.getItem("authToken");
    const url = roleId ? `/users?roleId=${roleId}` : "/users";
   ...
   * Update user by ID
   */
  updateUserById: async (id: string, data: Partial<UserData>) => {
    try {
      const token = localStorage.getItem("authToken");
   ...
   */
  adminChangeUserPassword: async (id: string, newPassword: string) => {
    try {
      const token = localStorage.getItem("authToken");
      const response = await userApi.put(`/users/${id}/change-password`,
        { newPassword },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      return response.data;
