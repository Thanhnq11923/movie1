...
import StaffBooking from "../../../components/Staff/member-booking/member-booking";

const MemberBooking: React.FC = () => {
  const [activeItem, setActiveItem] = useState("member-booking");
  ...
  const handleItemClick = (itemId: string) => {
    console.log(
      `Booking Staff - Clicked on: ${itemId}, current activeItem: ${activeItem}`
    );
  ...
  const toggleSidebar = () => {
    console.log(`Booking Staff - Toggling sidebar: ${isOpen} -> ${!isOpen}`);
    setIsOpen(!isOpen);
  };
  ...
  const getPageTitle = () => {
    const titles: Record<string, string> = {
      ...
      Members: "Members Management", // Keep both for compatibility
    };
    return titles[activeItem] || "Member Booking"; // ✅ FIXED: Removed extra space
  };

  const renderContent = () => {
    console.log(
      `Booking Staff - Rendering content for activeItem: ${activeItem}`
    );
    switch (activeItem) {
      case "booking":
        return <StaffBooking />;
    ...
    <div className="flex min-h-screen bg-gray-100 dark:bg-gray-900 w-full">
      ...
      {isOpen && isMobile && (
        <div
          ...
          onClick={() => {
            console.log("Booking Staff - Overlay clicked - closing sidebar");
            setIsOpen(false);
          }}
        />
      )}
