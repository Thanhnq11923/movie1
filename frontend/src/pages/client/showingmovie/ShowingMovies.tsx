...
} from "../../../components/movies/FilterTabs";
import MovieCard from "../../../components/movies/MovieCard";
import TrailerModal from "../../../components/movies/TrailerModal";

/**
 * Component ShowingMovie - Trang hiển thị danh sách phim đang chiếu
 * Tính năng chính:
...
export default function ShowingMovie() {
      ...
      ...new Set(
        movies.flatMap((movie) =>
          Array.isArray(movie.movieTypes)
            ? movie.movieTypes.map((t) => t.typeName)
            : []
        )
      ),
    ];
  ...
  const filteredMovies = movies.filter((movie) => {
    ...
    const matchesCategory =
      ...
      (Array.isArray(movie.movieTypes)
        ? movie.movieTypes.some(
            (type) => type.typeName === selectedFilters.category
          )
        : false);
    ...
    //
    const matchesStatus = movie.status?.toLowerCase().trim() === "showing";
              ...
              {filteredMovies.length > 0 ? (
                ...
                <div className="col-span-full text-center py-8 sm:py-12 md:py-16">
                  ...
                  <p className="text-sm sm:text-base text-gray-500 max-w-md mx-auto">
                    {searchQuery
                      ? `There are no movies that match the keyword "${searchQuery}"`
                      : "No movies found"}
                  </p>
                </div>
              )}
